...
final class MarketingHttpClient
{
    ...
    {
        if(!function_exists('curl_init'))throw new RuntimeException('A extensão cURL é obrigatória para integrações externas.');
        ...
        for($attempt=0;$attempt<=$maxRetries;$attempt++){
            ...
            if($response!==false&&$status<300){$decoded=json_decode($response,true);if(!is_array($decoded))throw new RuntimeException('A API externa retornou JSON inválido.');return$decoded;}
            if(($status===429||$status>=500||$response===false)&&$attempt<$maxRetries){usleep((int)(250000*(2**$attempt)));continue;}
            $decoded=is_string($response)?json_decode($response,true):null;$message=is_array($decoded)?($decoded['error']['message']??$decoded['message']??$decoded['error']['message_description']??'requisição recusada'):($transport?:'requisição recusada');throw new RuntimeException('Falha na API externa (HTTP '.$status.'): '.mb_substr((string)$message,0,500));
        }
        throw new RuntimeException('Falha na API externa após novas tentativas.');
    }
