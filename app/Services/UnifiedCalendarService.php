...
final class UnifiedCalendarService
{
    ...
    public function events(string $start, string $end): array
    {
        ...
        $queries = [
            ...
            ['external', "SELECT e.id,COALESCE(e.summary,'Reserva externa') AS title,e.starts_at,e.ends_at,e.status,s.provider AS source FROM calendar_external_events e JOIN calendar_sources s ON s.id=e.source_id WHERE e.starts_at<? AND e.ends_at>? AND e.status<>'CANCELLED' AND e.deleted_at IS NULL", [$end,$start]],
