...
test('reservas canceladas nao aparecem no calendario',function(){$service=file_get_contents(BASE_PATH.'/app/Services/UnifiedCalendarService.php');$controller=file_get_contents(BASE_PATH.'/app/Controllers/AdminController.php');foreach([$service,$controller]as$source)expect(str_contains($source,"status<>'CANCELADA'"));});
