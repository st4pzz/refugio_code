...
test('aprovacao envia cobranca por email imediatamente e informa o resultado real',function(){$service=file_get_contents(BASE_PATH.'/app/Services/ReservationService.php');$controller=file_get_contents(BASE_PATH.'/app/Controllers/AdminController.php');expect(str_contains($service,"customer(\$reservation, 'RESERVA_APROVADA'"));expect(str_contains($service,"['_delivery' => \$delivery]"));expect(str_contains($controller,"['_delivery']['email']"));expect(str_contains($controller,'e-mail enviado ao cliente'));expect(str_contains($controller,'mas o e-mail não pôde ser enviado'));});
...
test('secao de videos usa carrossel acessivel e mp4 otimizado', function() { $html=file_get_contents(BASE_PATH.'/index.php');$css=file_get_contents(BASE_PATH.'/style.css');$js=file_get_contents(BASE_PATH.'/assets/js/videos-carousel.js');foreach(['id="videos"','data-video-carousel','data-video-slide','controls playsinline preload="metadata"','assets/js/videos-carousel.js']as$needle)expect(str_contains($html,$needle));foreach(['.video-carousel','.video-frame','aspect-ratio: 9 / 16']as$needle)expect(str_contains($css,$needle));foreach(['aria-hidden','video.pause()','ArrowLeft','ArrowRight']as$needle)expect(str_contains($js,$needle));expect(is_file(BASE_PATH.'/assets/videos/IMG_0277.mp4'));expect(filesize(BASE_PATH.'/assets/videos/IMG_0277.mp4')<20*1024*1024); });
test('novas fotos otimizadas aparecem na galeria publica', function() { $html=file_get_contents(BASE_PATH.'/index.php');foreach(['IMG_6618.webp','IMG_6614.webp','foto_spa.webp','IMG_6608.webp','IMG_0553.webp','IMG_0550.webp','IMG_0549.webp','IMG_0544.webp','IMG_0308.webp']as$file){expect(str_contains($html,'assets/images/galeria/'.$file));expect(is_file(BASE_PATH.'/assets/images/galeria/'.$file));expect(filesize(BASE_PATH.'/assets/images/galeria/'.$file)<500*1024);} });
...
test('urls publicas exigidas pela meta possuem rotas e documentos completos',function(){$routes=file_get_contents(BASE_PATH.'/.htaccess');$policy=file_get_contents(BASE_PATH.'/politicas/index.php');$top=file_get_contents(BASE_PATH.'/app/Views/public/_top.php');$docs=file_get_contents(BASE_PATH.'/docs/META_APP_URLS.md');expect(str_contains($routes,'exclusao-de-dados'));foreach(['Política de Privacidade','Termos de Serviço','Exclusão de Dados do Usuário','Refugio_Site','Revogar o acesso pela Meta']as$needle)expect(str_contains($policy,$needle));expect(!str_contains($policy,'texto inicial editavel'));expect(str_contains($top,"\$robots ?? 'noindex,nofollow'"));foreach(['https://www.refugiodocuscuzeiro.com.br/politicas/privacidade','https://www.refugiodocuscuzeiro.com.br/politicas/termos','https://www.refugiodocuscuzeiro.com.br/politicas/exclusao-de-dados','https://www.refugiodocuscuzeiro.com.br/admin/configuracoes/integracoes/meta/callback','https://www.refugiodocuscuzeiro.com.br/api/whatsapp/webhook']as$needle)expect(str_contains($docs,$needle));});
...
test('avaliacoes externas manuais possuem origem e idempotencia',function(){
    $sql=file_get_contents(BASE_PATH.'/database/migrations/016_create_external_reviews.sql');
    foreach(['origem_plataforma','external_review_id','uk_avaliacao_origem_externa','expira_em']as$needle)expect(str_contains($sql,$needle));
    expect(is_file(BASE_PATH.'/database/migrations/016_drop_external_reviews.sql'));
    $transition=file_get_contents(BASE_PATH.'/database/migrations/017_create_google_places_live_transition.sql');
    expect(str_contains($transition,"DELETE FROM avaliacoes WHERE origem_plataforma='GOOGLE'"));
...
test('google places consulta cinco avaliacoes ao vivo sem oauth ou persistencia',function(){
    ...
    foreach(['places.googleapis.com/v1/places/','X-Goog-Api-Key','X-Goog-FieldMask','array_slice($payload[\'reviews\'] ?? [], 0, 5)','GOOGLE_PLACES_PLACE_ID']as$needle)expect(str_contains($service,$needle));
    foreach(['oauth2.googleapis.com','saveGoogleIntegration','upsertExternalReview([\n                    \'provider\' => \'GOOGLE\'']as$needle)expect(!str_contains($service,$needle));
    $env=file_get_contents(BASE_PATH.'/.env.example');
    foreach(['GOOGLE_PLACES_API_KEY','GOOGLE_PLACES_PLACE_ID']as$needle)expect(str_contains($env,$needle));
    expect(!str_contains($env,'GOOGLE_BUSINESS_'));
    $public=file_get_contents(BASE_PATH.'/app/Controllers/PublicReviewController.php');
    expect(str_contains($public,'fetchGooglePlacesReviews'));
    expect(str_contains($public,"Cache-Control: no-store"));
});
test('booking e airbnb entram manualmente com autorizacao origem e moderacao',function(){
    ...
    expect(str_contains($routes,'admin/avaliacoes/importar-manual'));
    expect(!str_contains($routes,'review-google-'));
...
test('vitrine identifica a plataforma da avaliacao externa sem inserir html',function(){
    ...
    foreach(["GOOGLE: 'Google Maps'","BOOKING: 'Booking.com'",'textContent']as$needle)expect(str_contains($script,$needle));
...
test('perfil super admin sempre recebe acesso total mesmo com json antigo',function(){$source=file_get_contents(BASE_PATH.'/app/Services/AuthorizationService.php');expect(str_contains($source,"\$roleCode === 'SUPER_ADMIN'"));expect(str_contains($source,"\$permissions['*'] = true"));expect(str_contains($source,"in_array('SUPER_ADMIN', (array) (\$_SESSION['admin_roles'] ?? []), true)"));});
...
test('imagem e audio sao inferidos quando o tipo do webhook esta ausente ou inconsistente', function() { [$imageType,,$imageId]=WhatsAppWebhookService::messageData(['image'=>['id'=>'image-1','mime_type'=>'image/jpeg']]);[$audioType,,$audioId]=WhatsAppWebhookService::messageData(['type'=>' unknown ','audio'=>['id'=>'audio-1','mime_type'=>'audio/ogg']]);expect($imageType==='IMAGEM');expect($imageId==='image-1');expect($audioType==='AUDIO');expect($audioId==='audio-1'); });
...
test('google ads script valida hmac timestamp e replay temporal', function() { $body='{"schema_version":1}';$secret=str_repeat('s',32);$now=1785900000;$timestamp=(string)$now;$signature='sha256='.hash_hmac('sha256',$timestamp.'.'.$body,$secret);expect(GoogleAdsScriptImportService::validSignature($body,$timestamp,$signature,$secret,$now));expect(!GoogleAdsScriptImportService::validSignature($body.'x',$timestamp,$signature,$secret,$now));expect(!GoogleAdsScriptImportService::validSignature($body,(string)($now-301),'sha256='.hash_hmac('sha256',($now-301).'.'.$body,$secret),$secret,$now)); });
test('google ads script limita campanhas e nao inclui segredo no download', function() { $names=GoogleAdsScriptImportService::campaignNames('Leads-Search-1, Leads-Performance Max-2,Leads-Search-1');expect($names===['Leads-Search-1','Leads-Performance Max-2']);$script=GoogleAdsScriptTemplate::render('https://www.refugiodocuscuzeiro.com.br/',$names);expect(str_contains($script,'https://www.refugiodocuscuzeiro.com.br/api/marketing/google-ads-script'));expect(str_contains($script,'Leads-Performance Max-2'));expect(str_contains($script,'COLE_AQUI_O_MESMO_SEGREDO_DO_ENV'));expect(!str_contains($script,'GOOGLE_ADS_SCRIPT_SECRET=')); });
test('endpoint google ads script e protegido por hmac e idempotencia', function() { $route=file_get_contents(BASE_PATH.'/.htaccess');$controller=file_get_contents(BASE_PATH.'/app/Controllers/GoogleAdsScriptWebhookController.php');$service=file_get_contents(BASE_PATH.'/app/Services/GoogleAdsScriptImportService.php');$sql=file_get_contents(BASE_PATH.'/database/migrations/012_create_google_ads_script_imports.sql');expect(str_contains($route,'api/marketing/google-ads-script'));foreach(['HTTP_X_REFUGIO_TIMESTAMP','HTTP_X_REFUGIO_SIGNATURE','validSignature']as$needle)expect(str_contains($controller,$needle));expect(str_contains($service,'beginTransaction()'));expect(substr_count($service,'ON DUPLICATE KEY UPDATE')>=3);expect(str_contains($sql,'UNIQUE KEY uk_google_ads_script_request')); });
test('sincronizacao oauth ignora contas administradas por google ads script', function() { $integration=['config_json'=>'{"connection_mode":"GOOGLE_ADS_SCRIPT"}'];expect(Refugio\Services\MarketingSyncService::isScriptManaged($integration));$cron=file_get_contents(BASE_PATH.'/scripts/sync_marketing.php');expect(str_contains($cron,'isScriptManaged')); });
...
test('eventos externos exibem label e cor do provedor',function(){$service=file_get_contents(BASE_PATH.'/app/Services/UnifiedCalendarService.php');$view=file_get_contents(BASE_PATH.'/app/Views/admin/calendar.php');$css=file_get_contents(BASE_PATH.'/assets/css/admin-calendar.css');expect(str_contains($service,'s.ativo=1'));expect(str_contains($view,'$label=$external?'));expect(str_contains($view,"provider-'.strtolower"));foreach(['provider-airbnb','provider-booking','provider-google']as$needle)expect(str_contains($css,$needle));});
test('calendario exibe somente reservas confirmadas sem duplicar bloqueio interno',function(){$service=file_get_contents(BASE_PATH.'/app/Services/UnifiedCalendarService.php');$controller=file_get_contents(BASE_PATH.'/app/Controllers/AdminController.php');$view=file_get_contents(BASE_PATH.'/app/Views/admin/calendar.php');expect(str_contains($service,"status='RESERVA_CONFIRMADA'"));expect(str_contains($service,"e.status='CONFIRMED'"));expect(!str_contains($service,"status<>'CANCELADA'"));expect(!str_contains($service,"e.status<>'CANCELLED'"));expect(!str_contains($service,"['reservation',"));expect(!str_contains($service,"['hold',"));expect(str_contains($service,"WHERE status='RESERVA_CONFIRMADA' ORDER BY checkin"));expect(str_contains($controller,'reserva_id IS NULL'));expect(str_contains($view,"\$label=\$external?(string)\$event['source']:'block'"));});
test('anexo dinamico respeita a capacidade configurada de hospedes',function(){$rows=ContractTemplateService::guestRows([['full_name'=>'Maria']],12);expect(substr_count($rows,'<tr>')===12);try{ContractTemplateService::guestRows(array_fill(0,13,['full_name'=>'X']),12);}catch(RuntimeException){return;}throw new RuntimeException('Mais de 12 hóspedes foram aceitos.');});
...
test('contrato pdf termina com folha de assinaturas para assinatura eletronica',function(){
    $variables=[
        'contract_city'=>'Analândia/SP','contract_date_long'=>'04/08/2026',
        'owner_full_name'=>'Locador Exemplo','owner_cpf'=>'12345678901',
        'guest_full_name'=>'Hóspede Exemplo','guest_cpf'=>'98765432100',
    ];
    $html=ContractPdfTemplate::render('Contrato CTR-TESTE','<p>Conteúdo contratual.</p>',str_repeat('a',64),$variables);
    expect(str_contains($html,'page-break-before: always'));
    expect(str_contains($html,'FOLHA DE ASSINATURAS'));
    expect(str_contains($html,'Gov.br'));
    expect(str_contains($html,'LOCADOR(A)'));
    expect(str_contains($html,'LOCATÁRIO(A)'));
    expect(str_contains($html,'Locador Exemplo · CPF 123.456.789-01'));
    expect(str_contains($html,'Hóspede Exemplo · CPF 987.654.321-00'));
    expect(substr_count($html,'TESTEMUNHA ')===2);
